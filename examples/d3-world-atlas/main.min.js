var app = (function (exports) {
  ...
  function cartesian(spherical) {
    ...
    return [cosPhi * cos$2(lambda), cosPhi * sin$1(lambda), sin$1(phi)];
  }
  ...
  function circleStream(stream, radius, delta, direction, t0, t1) {
    ...
    for (var point, t = t0; direction > 0 ? t > t1 : t < t1; t -= step) {
      point = spherical([cosRadius, -sinRadius * cos$2(t), -sinRadius * sin$1(t)]);
  ...
  function longitude(point) {
    if (abs$1(point[0]) <= pi$1)
      ...
      return sign(point[0]) * ((abs$1(point[0]) + pi$1) % tau - pi$1);
  }
  ...
  function polygonContains(polygon, point) {
    var lambda = longitude(point),
        phi = point[1],
        sinPhi = sin$1(phi),
        normal = [sin$1(lambda), -cos$2(lambda), 0],
        angle = 0,
        winding = 0;
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      for (var j = 0; j < m; ++j, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
        ...
        sum.add(atan2$2(k * sign * sin$1(absDelta), cosPhi0 * cosPhi1 + k * cos$2(absDelta)));
  ...
  function clipAntimeridianLine(stream) {
    ...
    return {
      ...
      point: function(lambda1, phi1) {
        ...
        if (abs$1(delta - pi$1) < epsilon) { // line crosses a pole
          ...
          if (abs$1(lambda0 - sign0) < epsilon) lambda0 -= sign0 * epsilon; // handle degeneracies
          if (abs$1(lambda1 - sign1) < epsilon) lambda1 -= sign1 * epsilon;
  ...
  function clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
    ...
    return abs$1(sinLambda0Lambda1) > epsilon
        ? atan((sin$1(phi0) * (cosPhi1 = cos$2(phi1)) * sin$1(lambda1)
            - sin$1(phi1) * (cosPhi0 = cos$2(phi0)) * sin$1(lambda0))
  ...
  function clipCircle(radius) {
    var cr = cos$2(radius),
        delta = 6 * radians$1,
        smallRadius = cr > 0,
        notHemisphere = abs$1(cr) > epsilon; // TODO optimise for this common case
    ...
    function intersect(a, b, two) {
      ...
      if (meridian
          ? polar
            ? phi0 + phi1 > 0 ^ q[1] < (abs$1(q[0] - lambda0) < epsilon ? phi0 : phi1)
            : phi0 <= q[1] && q[1] <= phi1
  ...
  function clipRectangle(x0, y0, x1, y1) {
    ...
    function corner(p, direction) {
      return abs$1(p[0] - x0) < epsilon ? direction > 0 ? 0 : 3
          : abs$1(p[0] - x1) < epsilon ? direction > 0 ? 2 : 1
          : abs$1(p[1] - y0) < epsilon ? direction > 0 ? 1 : 0
  ...
  function graticule() {
    ...
    function lines() {
      return range(ceil(X0 / DX) * DX, X1, DX).map(X)
          ...
          .concat(range(ceil(x0 / dx) * dx, x1, dx).filter(function(x) { return abs$1(x % DX) > epsilon; }).map(x))
          .concat(range(ceil(y0 / dy) * dy, y1, dy).filter(function(y) { return abs$1(y % DY) > epsilon; }).map(y));
    }
    ...
    function resampleLineTo(x0, y0, lambda0, a0, b0, c0, x1, y1, lambda1, a1, b1, c1, depth, stream) {
      ...
      if (d2 > 4 * delta2 && depth--) {
        var a = a0 + a1,
            ...
            phi2 = asin$2(c /= m),
            lambda2 = abs$1(abs$1(c) - 1) < epsilon || abs$1(lambda0 - lambda1) < epsilon ? (lambda0 + lambda1) / 2 : atan2$2(b, a),
  ...
  function projectionMutator(projectAt) {
    ...
    projection.clipAngle = function(_) {
      return arguments.length ? (preclip = +_ ? clipCircle(theta = _ * radians$1) : (theta = null, clipAntimeridian), reset()) : theta * degrees$3;
    };
    ...
    projection.angle = function(_) {
      return arguments.length ? (alpha = _ % 360 * radians$1, recenter()) : alpha * degrees$3;
    };
  ...
  function geoSatellite() {
    ...
    return p
        .scale(432.147)
        .clipAngle(acos(1 / distance) * degrees$2 - 1e-6);
  }
  ...
  function init(display, center, altitude) {
    ...
    const initialPos = (center && Array.isArray(center) && center.length === 2)
      ? [center[0] / degrees$1, center[1] / degrees$1]
      : [0.0, 0.0];
  ...
  function main() {
    ...
    var land110m, land50m, hiRes = false;
